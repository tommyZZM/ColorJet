...
(function (game) {
    var Cloud = (function (_super) {
        ...
        function Cloud(width, height) {
            ...
            this._cloud = new game.canvas.graphic.Rectangle(width, height);
...
(function (game) {
    var JetBird = (function (_super) {
        ...
        function JetBird() {
            ...
            this._bodybox = new game.sat.Box(new game.sat.Vector(0, 0), (this.display.width - 42) * this.display.scaleX, this.display.height * this.display.scaleY);
            this._body = this._bodybox.toPolygon();
            this.debugBody();
        }
        JetBird.prototype.debugBody = function () {
            var _this = this;
            if (!alcedo.core(game.GameCycler).debug)
                return;
            var debug = new game.canvas.graphic.Rectangle(this._bodybox.w / this.display.scaleX, this._bodybox.h / this.display.scaleY, "#e74c3c");
            //debug.pivotX = debug.pivotY = 0.5;
            debug.alpha = 0.3;
            this.display.addChild(debug);
            var boxbound = new game.canvas.DisplayGraphic();
            boxbound._graphicfn = function (ctx) {
                ctx.beginPath();
                for (var i = 0; i < _this.body.edges.length; i++) {
                    var next = i + 1;
                    if (next == _this.body.edges.length)
                        next = 0;
                    drawLineFromTo(ctx, { x: _this.body.edges[i].x + _this.body.pos.x, y: _this.body.edges[i].y + _this.body.pos.y }, { x: _this.body.edges[next].x + _this.body.pos.x, y: _this.body.edges[next].y + _this.body.pos.y });
                }
                ctx.stroke();
                ctx.lineWidth = 1;
    ...
    var WorldManager = (function (_super) {
        ...
        WorldManager.prototype.precheckEntities = function () {
            ...
            for (var i = 0; i < this._entities.length; i++) {
                ...
                else if (entitie instanceof game.Cloud) {
...
(function (game) {
    var ColourPower = (function (_super) {
        ...
        function ColourPower() {
            _super.call(this);
            this.velocitystatic = true;
            this._display = new game.canvas.graphic.Circle(10, "#e67e22");
        }
        Object.defineProperty(ColourPower.prototype, "fillColour", {
            set: function (c) {
                this._display.fillcolour = c;
            },
            enumerable: true,
            configurable: true
        });
...
(function (game) {
    var PlayGround = (function (_super) {
        ...
        PlayGround.prototype.startUp = function () {
            ...
            this._myplane.display.y = game.stage.stageHeight + 100;
            this._myplane.gravityenable = true;
            //debug
            if (alcedo.core(game.GameCycler).debug) {
            }
            alcedo.core(game.WorldManager).addEntity(this._myplane);
            alcedo.core(game.CameraManager).lookAt(this._myplane);
            alcedo.core(game.ParallaxManager).referenceObject(this._myplane);
            ...
            game.stage.addEventListener(alcedo.canvas.Stage.ENTER_MILLSECOND10, this.eachTime, this);
            ...
            alcedo.core(game.GameControl).startUp(this._myplane);
        };
        PlayGround.prototype.eachTime = function () {
            if (alcedo.core(game.GameState).isplaying) {
                //检测小鸟的位置
                if (this._myplane.y > game.stage.height && this._myplane.velocity.y > 0) {
                    //TODO：掉落云雾的特效
                    alcedo.dispatchCmd(game.GameState, game.GameState.OVER, { scorx: 0 });
                }
            }
        };
